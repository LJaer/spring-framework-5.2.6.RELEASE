...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		String beanName = id;
		// 如果 <bean> 元素中没有配置 id 属性，将别名的第一个值赋给 beanName
		...
		if (beanDefinition != null) {
			if (!StringUtils.hasText(beanName)) {
				try {
					if (containingBean != null) {
						// 如果 <bean> 元素中没有配置 id、name 或者别名，且没有包含子元素
						// <bean> 元素，则为解析的 Bean 生成一个唯一 beanName 并注册
						beanName = BeanDefinitionReaderUtils.generateBeanName(
								beanDefinition, this.readerContext.getRegistry(), true);
					}
					else {
						// 如果 <bean> 元素中没有配置 id、name 或者别名，且包含了子元素
						// <bean> 元素，则为解析的 Bean 使用别名向 IoC 容器注册 TODO 都没别名怎么注册
	...
	// 由于上面的方法以及对 Bean 的 id、name 和别名等属性进行了处理
	// 该方法中主要处理处着三个以外的其他属性
	@Nullable
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		// 判断属性值是 ref 还是 value ，不允许即使 ref 又是 value
		...
		// 如果当前 <property> 元素还有子元素
			...
			// 属性既不是 ref 也不是 value ，解析出错，返回 null
	...
	public Object parsePropertySubElement(Element ele, @Nullable BeanDefinition bd, @Nullable String defaultValueType) {
		...
		// 如果子元素是 ref，ref 只能有3个属性： bean、local、parent
